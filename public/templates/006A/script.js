document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', async function (event) {
      ...
      const promise = fetch('/template/006A', options); // inget ubah database
...
btnMusic.addEventListener("click", () =>{
    if(btnMusic.classList.contains("musicOn")){
        ...
        btnMusicImg.src ="../templates/006A/svg/MusicOff.svg"
        audio.pause()
    }else{
        btnMusic.classList.replace("musicOff", "musicOn")
        btnMusicImg.src ="../templates/006A/svg/MusicOn.svg"
        audio.play()
    }
})
